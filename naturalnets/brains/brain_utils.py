...
def validate_list_of_ints(instance, attribute, value):
    """
    A custom attrs validator, to validate that value is a list of integers
    """
    if isinstance(value, list):
        validate_failed = not all(isinstance(x, int) for x in value)
    else:
        validate_failed = True

    if validate_failed:
        raise ValueError(f"'{attribute.name}' must be a list of integers")
